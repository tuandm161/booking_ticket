...
class Booking {
  ...
  final List<OrderItem> productItems, comboItems;
  final String? voucherCode;
  final int subtotal, discountAmount, totalAmount;
  ...
  static List<SeatItem> _seatList(Object? value) => value is Iterable
      ? value
            ...
            .map(
              (x) =>
                  SeatItem.fromMap(x.map((k, v) => MapEntry(k.toString(), v))),
            )
            .toList()
      : <SeatItem>[];
  static List<OrderItem> _orderList(Object? value) => value is Iterable
      ? value
            ...
            .map(
              (x) =>
                  OrderItem.fromMap(x.map((k, v) => MapEntry(k.toString(), v))),
            )
            .toList()
      : <OrderItem>[];
